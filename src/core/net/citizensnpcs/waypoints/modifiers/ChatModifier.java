...
public class ChatModifier extends WaypointModifier {
    ...
    public boolean converse(Player player, ConversationMessage message) {
        ...
        if (StringUtils.isCommand(message.getMessage())) {
            ...
            messages.add(text);
            player.sendMessage(ChatColor.GREEN + "Added " + StringUtils.wrap(ChatColor.stripColor(text))
                    + ".");
        }
        return false;
    }
    ...
    public void parse(Storage storage, String root) {
        for (String string : Splitter.on(",").split(storage.getString(root + ".messages"))) {
            messages.add(string);
        }
    }
    ...
    public void save(Storage storage, String root) {
        storage.setString(root + ".messages", StringUtils.join(messages, ","));
    }
